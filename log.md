...
## [2026-07-29] 修复模型切换后的相机裁剪与取景距离

### 用户提出的问题
1. 部分 STL 在拉近时出现残缺、拉远后恢复显示。
2. 在 STL 与 URDF/Xacro 之间切换时，URDF/Xacro 会继承不合适的观察距离并显示过近。

### 问题原因/修改思路
- 固定的相机近裁剪面会在近距离查看网格时裁掉靠近镜头的三角面；根据相机到观察目标的距离动态调整 near/far，可同时保持近距离查看与深度缓冲精度。
- 独立 CAD 模型已有自动取景，而 URDF/Xacro 加载后未重置相机；改为每个新加载模型统一自动取景，避免不同格式之间继承相机状态。

### 实际修改记录
- **`src/components/Viewer.tsx`**：在渲染循环中依据 OrbitControls 的相机距离动态更新近远裁剪面，并保留 1% 阈值避免无效投影矩阵更新。
- **`src/App.tsx`**：对所有非空加载模型启用自动取景，而不再仅对独立 CAD 模型启用。
