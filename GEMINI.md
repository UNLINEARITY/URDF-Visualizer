...
*   **统一 Z-UP 环境**: 为了符合机器人学标准，本项目已全面切换为 **Z-UP** 坐标系环境。
    *   **世界背景**: 全局网格 (Grid) 位于 XY 平面，蓝色 Z 轴垂直指向上方。
    *   **坐标准则**: 遵循 **FLU (Forward-Left-Up)** 规范，红色 X 轴指向前方，绿色 Y 轴指向左侧。
    *   **无需手动旋转**: 加载的 Robot 对象直接以原始 Z-up 姿态进入场景，确保 3D 视觉与机器人控制器指令完全一致。

### 4.2 交互式拾取 (Picking)
我们没有使用简单的 Mesh 拾取，而是实现了**层级回溯拾取**：
1.  **穿透搜索**: 射线检测会遍历所有交点，自动过滤掉 `helper` 和 `axes` 对象，直到锁定真正的 `URDFLink`。
2.  **视觉反馈**: 鼠标悬停在可交互零件上时，光标会自动变为 `pointer`（小手），提供即时反馈。
3.  **锁定逻辑**: 向上遍历 `object.parent` 检查 `isURDFLink` 或 `isURDFJoint` 属性，锁定整个逻辑部件。
...
*   **RPY 分解**: 姿态数据采用 **ZYX (Yaw-Pitch-Roll)** 旋转顺序进行分解。
    *   **Roll**: 绕红色 X 轴旋转（侧倾）。
    *   **Pitch**: 绕绿色 Y 轴旋转（点头）。
    *   **Yaw**: 绕蓝色 Z 轴旋转（原地转弯）。
*   **Global (World)**: 显示部件相对于世界原点的绝对位置。由于环境已统一为 Z-up，此数据即为真实的物理空间坐标。
...
### 4.12 1:1 像素投影拖拽算法 (核心交互)
... (内容已在上面替换) ...
*   **限位保护**: 拖拽过程中实时校验并强制执行 URDF 定义的 `limit` 软硬限位。

### 4.13 智能资源路径容错 (新增)
针对不同来源的 URDF 包结构（特别是非标准的单层结构），实现了智能重定向机制：
*   **自动回溯**: 如果模型文件位于 `urdf/` 目录但 mesh 路径不含 `../`，系统会自动尝试回溯到包根目录寻找。
*   **多格式支持**: 集成了 `STLLoader`, `ColladaLoader` (.dae), 和 `OBJLoader`。
*   **安全加载**: 在尝试解析二进制 mesh 之前，先进行 HTTP 状态检查，避免因 404 HTML 页面注入导致的 STLLoader 内存溢出。
